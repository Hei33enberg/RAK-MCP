...
The contract is [`SPEC.md`](./SPEC.md). The economics are [`ECONOMICS.md`](./ECONOMICS.md). The **first** reference node is [rak.ad](https://rak.ad) — a live index of Poland's local media (4,800+ monitored sources, 16 regions): the proof, not the point. Everything here is MIT. The node is one implementation; the language — and the movement — are for every country.
