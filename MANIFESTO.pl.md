...
Kontraktem jest [`SPEC.md`](./SPEC.md). Ekonomią — [`ECONOMICS.md`](./ECONOMICS.md). **Pierwszym** węzłem referencyjnym jest [rak.guru](https://rak.guru): żywy indeks polskich mediów lokalnych (4800+ monitorowanych źródeł, 16 regionów) — dowód, nie cel. Wszystko tutaj jest MIT. Węzeł to jedno wdrożenie; język — i ruch — są dla każdego kraju.
