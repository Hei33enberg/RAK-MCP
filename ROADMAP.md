...
- **The first node (Poland)** — anonymous reader/discovery tools over MCP, live and free: 4,800+ monitored sources, 16 regions, hybrid RAG, wire feed, health — the proof other countries fork.
- **Owned lane** — `rak_owned_*`: publish · list · get · verify your own content objects with a portable content wallet (HMAC today).
- **Chat surface** — the RAK app ships a built-in community chat as an in-app client feature (not part of the public MCP toolset).
...
- **Native chat, deeper** — the in-app community chat could grow toward the fuller RFC-0001 vision (richer moderation, federation, community chat running on RAK itself), if pursued. Not exposed as public MCP tools. Host persona is the Crown's choice (none hardcoded).
